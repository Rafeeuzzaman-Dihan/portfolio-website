...
- `--color-live` — green, **status only**: live / online (Projects' LIVE bar, tab and Visit site button)
- `--color-alert` — amber, **status only**: classified / in progress (hazard-stripe bar, Classified panel, in-progress version)
...
### Projects section (approved and implemented in `app/components/sections/ProjectsSection.vue`)

- Real projects in `content/projects.json` (six, in the owner's order): Trip and Travel, FC Clubs Analytics, Clar B2B E-commerce, VP Clubs 11 (vpclubs11.com), Bangladesh Dental College (bdc.edu.bd), Shams Hardware & Tools (shamshtlbd.com). Each has `category`, `summary`, `image`, `status` (`live` or `classified`; there is **no private status**), `liveUrl`, `started`, `releases[{version, date, inProgress}]`, `tech` (names that must exist in `content/skills.json`, which supplies logo and brand colour) and a `caseStudy` (client, role, overview, challenge, built, underTheHood, outcome). Classified work never names the client. All images use the placeholder `/images/post.png` until the owner supplies screenshots. Project screenshots are allowed; the "no photos" rule is about the owner's portrait.
- Layout is an **at-a-glance card grid** (1 / 2 / 3 columns), 6 per page, with page buttons only when there are more than six. Every card shows its screenshot, its LIVE / CLASSIFIED status, category, latest release, title, summary, tech chips with brand logos and the two actions. **No filters** (All/Client/Personal were removed by the owner) and **no "INTEL 01"-style numbering**.
- Motion (approved as "Decrypt + Selector, Lift"): when the section scrolls into view, the cards decrypt in a wave (scanline screen and progress bar, the screenshot wipes in with a blue flash, the title decodes from 0s and 1s, then summary, tech and actions rise in). Then, on `lg`+ only, a selector frame glides between cards every 2.5s: the selected card **lifts** (whole card springs up ~12px), its corner brackets lock on and a timer bar runs; the other screenshots are muted. Once the visitor hovers, focuses or clicks a card, the auto-scan stops for good. **Screenshots never zoom or pan.** Reduced motion shows everything finished with no auto-scan. The intro stages run on timers so they always complete.
- **Colour means something** (approved option A, "status bar"): blue is only for actions (Case study button, selector frame); green (`--color-live`) means live; amber (`--color-alert`) means classified or in progress. The category label is grey, not blue.
- Status is a **4px bar along the card's top edge**, like a CoD rarity strip: solid glowing green when live, amber-and-black hazard stripes when classified. A tab hangs from it onto the screenshot: "● LIVE" (green) or "🔒 CLASSIFIED" (amber).
- The release is a **version track**: a filled white dot per shipped version joined by a line, a hollow amber pinging dot for a version in progress, then "V2 · Sep 2026" (white) or "V2 · In progress" (amber).
- Actions: an outlined blue **Case study** button that floods with blue from the left while its arrow slides forward, plus a solid green **● Visit site ↗** button when there's a public link, or an amber **🔒 Classified** panel when there isn't.
- Card details stay **written out in full** so non-developers understand them: status words, version and date, the full summary, and tech chips with **logo and name**. The owner rejected an icon-only version (lock-only badge, version-only chip, logo-only tech, icon-only Live button, clamped summary) as too cut down, and found an all-blue card too monotonous.
- Case studies open in a slide-in panel (`projects/CaseStudy.vue`) on the same page (no routes): screenshot, decoded title, a fact grid (client, role, timeline, status in green/amber), Overview, The challenge, What I built, Under the hood (with tech logos), Outcome, then the green Visit site button (or the amber classified note) and Next case. Escape closes it; focus is kept inside and returned afterwards.
- Rejected for this section (don't reintroduce): case-file tabs with stamps, showreel/editor timeline, viewfinder rows, level-select carousel, flagship + archive index, CoD menu tiles, mission-select list + briefing, blueprint and dossier-row card variants, cursor-tilt "target lock", hop and focus card motions, keycap and prompt-bar action buttons, category filters, a "private" status, solid status tags on the screenshot and a HUD status strip (options B and C).
