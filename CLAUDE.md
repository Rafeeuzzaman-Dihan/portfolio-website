...
- Real projects in `content/projects.json` (six, in the owner's order): Trip and Travel, FC Clubs Analytics, Clar B2B E-commerce, VP Clubs 11 (vpclubs11.com), Bangladesh Dental College (bdc.edu.bd), Shams Hardware & Tools. Each has `category`, `summary`, `image`, `status` (`live` / `classified` / `private`), `liveUrl`, `lockLabel`, `started`, `releases[{version, date, inProgress}]`, `tech` (names that must exist in `content/skills.json`, which supplies logo and brand colour) and a `caseStudy` (client, role, overview, challenge, built, underTheHood, outcome). Classified work never names the client. All images use the placeholder `/images/post.png` until the owner supplies screenshots. Project screenshots are allowed; the "no photos" rule is about the owner's portrait.
- Layout is an **at-a-glance card grid** (1 / 2 / 3 columns), 6 per page, with page buttons only when there are more than six. Every card shows its screenshot, a LIVE / CLASSIFIED / PRIVATE tag, category, latest release ("v2 · Sep 2026" or "v2 · In progress"), title, summary, tech chips with brand logos and the two actions. **No filters** (All/Client/Personal were removed by the owner) and **no "INTEL 01"-style numbering**.
...
- Actions (approved "Sweep" style): an outlined **Case study** button that floods with blue from the left while its arrow slides forward, plus a **● Live ↗** tab when there's a public link, or a dashed lock tab with `lockLabel` ("Classified", "Link soon") when there isn't.
- Case studies open in a slide-in panel (`projects/CaseStudy.vue`) on the same page (no routes): screenshot, decoded title, a fact grid (client, role, timeline, status), Overview, The challenge, What I built, Under the hood (with tech logos), Outcome, then See it live (or the lock note) and Next case. Escape closes it; focus is kept inside and returned afterwards.
- Rejected for this section (don't reintroduce): case-file tabs with stamps, showreel/editor timeline, viewfinder rows, level-select carousel, flagship + archive index, CoD menu tiles, mission-select list + briefing, blueprint and dossier-row card variants, cursor-tilt "target lock", hop and focus card motions, keycap and prompt-bar action buttons, category filters.
